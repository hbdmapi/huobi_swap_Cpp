    ...
    public:
        ...
        static void checkResponse(const JsonWrapper& json) {
            try {
                if (json.containKey("status")) {
                    ...
                    if ("error" == status) {
                        std::string err_code = json.getString("err_code");
                        std::string err_msg = json.getString("err_msg");
                        throw Error(Error::EXEC_ERROR,
                                "[Executing] " + err_code + ": " + err_msg);
                    } else if ("ok" != status) {
                        throw Error(
                                Error::RUNTIME_ERROR, "[Invoking] Response is not expected: " + status);
                    }
                    ...
                    throw Error(
                            Error::RUNTIME_ERROR, "[Invoking] Status cannot be found in response.");
                }
            } catch (std::exception e) {
                std::string msg;
                msg = "[Invoking] Unexpected error: ";
                throw Error(
                        Error::RUNTIME_ERROR, msg + e.what());
            }
        }
        ...
        static T callSync(RestApi<T>* request) {
            ...
            if (code != CURLE_OK) {
                ...
                throw Error("", "curl_global_init() Err");
            }
            CURL* pCurl = curl_easy_init();
            if (pCurl == NULL) {
                std::cout << "curl_easy_init() Err" << std::endl;
                throw Error("", "curl_easy_init() Err");
            }
            std::string sBuffer;
            printf("\n");
            printf("------request------\n");
            ...
            curl_easy_setopt(pCurl, CURLOPT_CAINFO, "/etc/HuobiSwap_cert/cert.pem");
            if (ptr->method == "POST") {
            ...
            if (code != CURLE_OK) {
                ...
                throw Error("", "curl_easy_perform() Err");
            }
            printf("------response------\n");
